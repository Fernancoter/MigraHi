...
import { LucidePencil, LucideX, LucideTrash2, LucideFolder, LucideSave, LucideFileText, LucideDownload, LucideSettings, LucideInfo } from '@lucide/angular';

@Component({
  selector: 'app-reclamos-list',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, LucidePencil, LucideX, LucideTrash2, LucideFolder, LucideSave, LucideFileText, LucideDownload, LucideSettings, LucideInfo],
  template: `
        ...
        <table class="gx-table">
          ...
          <tbody>
            <tr *ngFor="let r of paginatedReclamos; let i = index">
              ...
              <td class="action-cell"><span class="icon-gear"><svg lucideSettings [size]="14"></svg></span></td>
              <td class="action-cell">
                <a [routerLink]="['/calidad/reclamos', r.id]" class="icon-info"><svg lucideInfo [size]="14"></svg></a>
              </td>
...
export class ReclamosListComponent implements OnInit {
  ...
  saveActiveFilters() {
    ...
    if (name) {
      this.savedFilters.push({ id: Date.now(), name, term: this.filterCodigo });
    }
    this.showSearchFilterDropdown = false;
  }

  loadSavedFilter(f: any) {
    this.filterCodigo = f.term || '';
    this.aplicarFiltros();
    this.showSearchFilterDropdown = false;
  }
