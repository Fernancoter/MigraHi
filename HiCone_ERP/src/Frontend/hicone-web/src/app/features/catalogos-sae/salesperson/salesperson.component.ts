          ...
          <table class="wwp-table">
            ...
            <tbody>
              <tr *ngFor="let s of pagedData">
                <td class="action-cell"><button class="btn-icon" (click)="editRecord(s)">✏️</button></td>
                <td class="action-cell"><button class="btn-icon" (click)="deleteRecord(s)">✖️</button></td>
...
export class SalesPersonComponent implements OnInit {
  ...
  deleteRecord(s: SaeSalesPerson) {
    if (confirm(`¿Eliminar al vendedor "${s.salesPersonName}"?`)) {
      this.data = this.data.filter(x => x.salesPersonName !== s.salesPersonName);
      this.applyFilterAndSort();
    }
  }

  confirmRecord() {
    if (!this.formData.salesPersonName) { alert('El campo Person Name es requerido'); return; }
    if (this.isEditing) {
