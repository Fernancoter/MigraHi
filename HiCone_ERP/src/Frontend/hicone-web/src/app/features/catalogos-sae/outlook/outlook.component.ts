  ...
  template: `
    <div class="wwp-container">
      <div class="wwp-header">
        <h1>outlookww</h1>
        <div class="wwp-breadcrumbs">Reportes &gt; Outlook</div>
      </div>

      <div class="wwp-content">
        <h2 class="sub-title">Update Outlook</h2>
...
export class OutlookComponent implements OnInit {
  ...
  buildMatrix() {
    ...
    // Base customers filtered by group if selected
    ...
    this.originalBudgets.forEach(b => {
      if (map.has(b.customerCode)) {
        map.get(b.customerCode).months[b.budgetMonth] = b.budgetOutlook;
      }
    });
  ...
  saveData() {
    ...
    this.matrixData.forEach(row => {
      for (let m = 1; m <= 12; m++) {
        ...
        // Find existing to preserve other values like BudgetEstimated, Real, etc.
