...
@Component({
    ...
    <div class="module-page animate-move-up" (click)="closeAllDropdowns()">
      ...
      <div class="page-header-premium">
        <div class="title-section">
          <h1 class="premium-title">Catálogo de Turnos</h1>
          <nav class="breadcrumb-modern">
            <span class="root">Producción</span>
            <span class="sep">&rsaquo;</span>
            <span class="root">Catálogos</span>
            <span class="sep">&rsaquo;</span>
            <span class="active">Turnos</span>
          </nav>
        </div>
      </div>
                  ...
                  <div class="col-groups-container">
                    ...
                    <div class="col-group">
                      ...
                      @if (colNoFijasExpanded) {
                        <div class="col-group-body">
                          @if (!colSearch || 'turno'.includes(colSearch.toLowerCase())) {
                            <label class="col-item">
                              <input type="checkbox" [checked]="isTempColVisible('nombre')" (change)="toggleTempCol('nombre')"> Turno
                            </label>
                          }
                          @if (!colSearch || 'hora inicio'.includes(colSearch.toLowerCase())) {
                            <label class="col-item">
                              <input type="checkbox" [checked]="isTempColVisible('horaInicio')" (change)="toggleTempCol('horaInicio')"> Hora Inicio
                            </label>
                          }
                          @if (!colSearch || 'hora fin'.includes(colSearch.toLowerCase())) {
                            <label class="col-item">
                              <input type="checkbox" [checked]="isTempColVisible('horaFin')" (change)="toggleTempCol('horaFin')"> Hora Fin
                            </label>
                          }
                        </div>
                      }
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              @if (isColVisible('nombre')) {
                <th>
                  <div class="th-content">
                    <span>Turno</span>
                    <div class="dropdown-wrapper">
                      <button class="th-menu-btn" (click)="toggleColMenu('nombre', $event)">▾</button>
                      @if (activeColMenu() === 'nombre') {
                        <div class="col-header-menu animate-slide-up" (click)="$event.stopPropagation()">
                          <div class="dd-item" (click)="sortBy('nombre', 'asc')">↑ Ordenar de A a Z</div>
                          <div class="dd-item" (click)="sortBy('nombre', 'desc')">↓ Ordenar de Z a A</div>
                          <div class="menu-divider"></div>
                          <div class="dd-item">⇤ Fijar a la izquierda</div>
                          <div class="dd-item">⇥ Fijar a la derecha</div>
                          <div class="menu-divider"></div>
                          <div class="col-search-box" style="padding:.4rem;">
                            <input type="text" placeholder="Buscar..." [(ngModel)]="colFilterNombre" class="col-search-input" (click)="$event.stopPropagation()" />
                          </div>
                          <div style="padding:.3rem .8rem;font-size:.75rem;color:#94a3b8;">
                            @for (v of uniqueColValues('nombre'); track v) {
                              <div style="padding:.2rem 0;">{{ v }} ({{ countCol('nombre', v) }})</div>
                            }
                          </div>
                        </div>
                      }
              ...
              @if (isColVisible('horaInicio')) {
                <th>
                  <div class="th-content">
                    <span>Hora Inicio</span>
                    <div class="dropdown-wrapper">
                      <button class="th-menu-btn" (click)="toggleColMenu('horaInicio', $event)">▾</button>
                      @if (activeColMenu() === 'horaInicio') {
                        <div class="col-header-menu animate-slide-up" (click)="$event.stopPropagation()">
                          <div class="dd-item" (click)="sortBy('horaInicio', 'asc')">↑ Ordenar A-Z</div>
                          <div class="dd-item" (click)="sortBy('horaInicio', 'desc')">↓ Ordenar Z-A</div>
                          <div class="menu-divider"></div>
                          <div class="dd-item">⇤ Fijar a la izquierda</div>
                          <div class="dd-item">⇥ Fijar a la derecha</div>
                          <div class="menu-divider"></div>
                          <div class="col-search-box" style="padding:.4rem;">
                            <input type="text" placeholder="Buscar..." class="col-search-input" (click)="$event.stopPropagation()" />
                          </div>
                          <div style="padding:.3rem .8rem;font-size:.75rem;color:#94a3b8;">
                            @for (v of uniqueColValues('horaInicio'); track v) {
                              <div style="padding:.2rem 0;">{{ v }} ({{ countCol('horaInicio', v) }})</div>
                            }
                          </div>
                        </div>
                      }
              ...
              @if (isColVisible('horaFin')) {
                <th>
                  <div class="th-content">
                    <span>Hora Fin</span>
                    <div class="dropdown-wrapper">
                      <button class="th-menu-btn" (click)="toggleColMenu('horaFin', $event)">▾</button>
                      @if (activeColMenu() === 'horaFin') {
                        <div class="col-header-menu animate-slide-up" (click)="$event.stopPropagation()">
                          <div class="dd-item" (click)="sortBy('horaFin', 'asc')">↑ Ordenar A-Z</div>
                          <div class="dd-item" (click)="sortBy('horaFin', 'desc')">↓ Ordenar Z-A</div>
                          <div class="menu-divider"></div>
                          <div class="dd-item">⇤ Fijar a la izquierda</div>
                          <div class="dd-item">⇥ Fijar a la derecha</div>
                        </div>
          ...
          <tbody>
            @if (loading()) {
              <tr><td colspan="6" class="empty-state">Cargando turnos...</td></tr>
            } @else if (paginatedItems().length === 0) {
              <tr><td colspan="6" class="empty-state">No se encontraron turnos registrados</td></tr>
            } @else {
              @for (item of paginatedItems(); track item.id) {
                <tr>
                  ...
                  @if (isColVisible('horaInicio')) { <td>{{ item.horaInicio }}</td> }
                  @if (isColVisible('horaFin'))   { <td>{{ item.horaFin }}</td> }
                </tr>
              }
            }
          </tbody>
          ...
          <div class="modal-card erp-modal" (click)="$event.stopPropagation()">
            <div class="erp-panel">
              ...
              <div class="erp-panel-body">
                ...
                <!-- Turno -->
                <div class="erp-field">
                  <label class="erp-label">Turno</label>
                  <input class="erp-input" type="text" [(ngModel)]="form.nombre" placeholder="Ej. 1er Turno" />
                </div>
            ...
            <div class="erp-footer">
              <button class="btn-erp-confirm" type="button" (click)="save()">CONFIRMAR</button>
              <button class="btn-erp-cancel"  type="button" (click)="closeModal()">CANCELAR</button>
            </div>
          </div>
        </div>
      }
          ...
          <div class="modal-card viz-modal" (click)="$event.stopPropagation()">
            ...
            <div class="erp-panel">
              ...
              <div class="erp-panel-body">
                <div class="erp-field">
                  <label class="erp-label">Clave</label>
                  <div class="erp-readonly-value">{{ form.id || 'N/A' }}</div>
                </div>
                <div class="erp-separator"></div>
                <div class="erp-field">
                  <label class="erp-label erp-label-blue">Turno</label>
                  <div class="erp-readonly-value erp-value-blue">{{ form.nombre }}</div>
                </div>
                <div class="erp-field">
                  <label class="erp-label erp-label-blue">Hora Inicio</label>
                  <div class="erp-time-wrapper">
                    <div class="erp-readonly-value erp-value-blue">{{ form.horaInicio }}</div>
                    <span class="erp-cal-icon">📅</span>
                  </div>
                </div>
                <div class="erp-field">
                  <label class="erp-label erp-label-blue">Hora Fin</label>
                  <div class="erp-time-wrapper">
                    <div class="erp-readonly-value erp-value-blue">{{ form.horaFin }}</div>
                    <span class="erp-cal-icon">📅</span>
                  </div>
                </div>
              </div>
              <div class="erp-panel-footer">
      ...
      @if (showDeleteModal()) {
        <div class="modal-overlay" style="z-index:1100;" (click)="closeDeleteModal()">
          <div class="modal-card confirm-modal" (click)="$event.stopPropagation()">
            ...
            <div class="confirm-body"><p>¿Está seguro de eliminar el turno?</p></div>
  ...
  styles: [`
    ...
    /* th with sort button */
    .th-content { display:flex; align-items:center; gap:.3rem; }
    .th-menu-btn { background:none; border:none; cursor:pointer; color:#94a3b8; font-size:.75rem; padding:.1rem .2rem; border-radius:2px; }
    .th-menu-btn:hover { color:#475569; background:#e2e8f0; }
    .col-header-menu { position:absolute; top:110%; left:0; min-width:200px; background:white; border:1px solid #cbd5e1; border-radius:4px; box-shadow:0 6px 16px rgba(0,0,0,.12); z-index:300; }
    .menu-divider { border-top:1px solid #e2e8f0; margin:.3rem 0; }
    ...
    .action-btn.delete:hover { color:#b91c1c; text-decoration:underline; }
    ...
    .erp-modal  { width:700px; max-width:95vw; display:flex; flex-direction:column; gap:0; }
    .viz-modal  { width:95%; max-width:1000px; max-height:90vh; overflow-y:auto; display:flex; flex-direction:column; gap:0; }
    ...
    .erp-input-blue:focus { border-bottom-color:#1d72b8; }

    .erp-time-wrapper { display:flex; align-items:center; gap:.4rem; }
    .erp-cal-icon { color:#9ca3af; font-size:.9rem; flex-shrink:0; }
    .erp-cal-btn { background:none; border:none; cursor:pointer; font-size:.9rem; color:#9ca3af; padding:0; flex-shrink:0; }
    .erp-cal-btn:hover { color:#4b96d1; }
...
export class TurnosCatalogoComponent implements OnInit {
  ...
  itemToDelete     = signal<Turno | null>(null);
  ...
  visibleColumns       = signal<string[]>(['nombre', 'horaInicio', 'horaFin']);
  tempVisibleColumns   = signal<string[]>(['nombre', 'horaInicio', 'horaFin']);
  colSearch            = '';
  colFilterNombre      = '';
  colFijasIzqExpanded  = true;
  colNoFijasExpanded   = true;
  colFijasDerExpanded  = true;
  activeColMenu        = signal<string>('');
  ...
  filteredItems = computed(() => {
    ...
    if (s) list = list.filter(t => t.nombre.toLowerCase().includes(s));
    if (this.colFilterNombre) {
      list = list.filter(t => t.nombre.toLowerCase().includes(this.colFilterNombre.toLowerCase()));
    }
    const f = this.sortField(), d = this.sortDirection();
    list = [...list].sort((a, b) => {
  ...
  sortBy(field: string, dir: 'asc' | 'desc') {
    this.sortField.set(field); this.sortDirection.set(dir);
    this.activeColMenu.set('');
  }

  uniqueColValues(field: string): string[] {
    return [...new Set(this.filteredItems().map(t => (t as any)[field] ?? ''))];
  }
  countCol(field: string, val: string): number {
    return this.filteredItems().filter(t => (t as any)[field] === val).length;
  }

  toggleColMenu(col: string, event: Event) {
    event.stopPropagation();
    this.activeColMenu.update(v => v === col ? '' : col);
  ...
  resetColumns() { this.tempVisibleColumns.set(['nombre', 'horaInicio', 'horaFin']); }
  applyColumns() { this.visibleColumns.set([...this.tempVisibleColumns()]); this.showColumnSelector.set(false); }

  clearFilters() { 
    this.searchText.set(''); 
    this.colFilterNombre = ''; 
    this.currentPage.set(1); 
    this.showFilterOptions.set(false); 
  }

  loadSavedFiltersFromStorage() {
    const raw = localStorage.getItem('hicone_saved_filters_turnos');
    this.savedFilters = raw ? JSON.parse(raw) : [];
  }

  saveFilter() {
    this.showFilterOptions.set(false);
    const filterName = prompt('Ingrese el nombre para este filtro:', 'Filtro Turnos ' + new Date().toLocaleDateString());
    ...
    localStorage.setItem('hicone_saved_filters_turnos', JSON.stringify(this.savedFilters));
  ...
  openEditModal(item: Turno) {
    this.form = { ...item };
    this.selectedClaveId = item.id;
    this.modalReadOnly.set(false);
    this.showModal.set(true);
    this.closeAllDropdowns();
  }

  openViewModal(item: Turno) {
    this.form = { ...item };
  ...
  executeDelete() {
    ...
    if (!item) return;
    this.svc.deleteTurno(item.id).subscribe(() => {
  ...
  exportPDF() {
    ...
    if (!win) return;
    let heads = '';
    if (this.isColVisible('nombre'))    heads += '<th>Turno</th>';
    ...
    this.filteredItems().forEach(t => {
      ...
      if (this.isColVisible('horaInicio')) rows += `<td>${t.horaInicio}</td>`;
      if (this.isColVisible('horaFin'))   rows += `<td>${t.horaFin}</td>`;
      rows += '</tr>';
    });
    win.document.write(`<html><head><title>Turnos</title>
      ...
      </head><body><h1>Reporte de Turnos</h1><p>Generado: ${new Date().toLocaleString()}</p>
