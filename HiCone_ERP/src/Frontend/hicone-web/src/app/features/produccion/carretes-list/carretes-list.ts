...
export class CarretesListComponent implements OnInit {
  ...
  loadSavedFiltersFromStorage() {
    const raw = localStorage.getItem('hicone_saved_filters_carretes');
  ...
  saveFilter() {
    this.isFilterMenuOpen.set(false);
    const filterName = prompt('Ingrese el nombre para este filtro:', 'Filtro Carretes ' + new Date().toLocaleDateString());
