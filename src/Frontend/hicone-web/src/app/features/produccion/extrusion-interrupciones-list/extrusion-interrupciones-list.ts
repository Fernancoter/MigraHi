              ...
              @for (item of paginatedItems(); track item.id; let idx = $index) {
                <tr>
                  ...
                  @if (isColVisible('extrusora')) { 
                    <td style="color: #10b981; font-weight: 700; cursor: pointer;" (click)="verExtrusionDetalle(item)">
                      <span class="action-link-green">{{ item.extrusora }}</span>
                    </td> 
                  }
...
export class ExtrusionInterrupcionesListComponent implements OnInit {
  ...
  verExtrusionDetalle(item: any) {
    this.router.navigate(['/produccion/extrusiones'], { queryParams: { detailId: item.extrusionId } });
  }
  ...
  loadSavedFiltersFromStorage() {
    const raw = localStorage.getItem('hicone_saved_filters_ext_interrupc');
  ...
  saveFilter() {
    this.isFilterMenuOpen.set(false);
    const filterName = prompt('Ingrese el nombre para este filtro:', 'Filtro Interrupciones ' + new Date().toLocaleDateString());
