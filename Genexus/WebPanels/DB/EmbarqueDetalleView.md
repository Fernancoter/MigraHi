# WebPanel: EmbarqueDetalleView

- **Module:** DB
- **Description:** Embarque Detalle View
- **GAM Object:** No

## Data Dictionary / Parameters

| Name | Element Type | Data Type | Accessor | Description |
|---|---|---|---|---|
| IsAuthorized | Variable | Boolean |  | Is Authorized |
| WWPContext | Variable | GX_SDT |  | WWPContext |
| SecurityFunctionalityKeys | Variable | VARCHAR |  | Security Functionality Keys |
| TabCode | Variable | CHARACTER |  | Tab Code |
| Exists | Variable | Boolean |  | Exists |
| EmbarqueDetalleId | Variable | NUMERIC |  | Embarque Detalle Id |
| LoadAllTabs | Variable | Boolean |  | Load All Tabs |
| SelectedTabCode | Variable | CHARACTER |  | Selected Tab Code |
| TempBoolean | Variable | Boolean |  | Temp Boolean |
| AuditTableName | Variable | VARCHAR |  | Audit Table Name |
| Today | Variable | DATE |  | Today |
| Time | Variable | CHARACTER |  | Time |
| Pgmname | Variable | CHARACTER |  | Pgmname |
| Pgmdesc | Variable | CHARACTER |  | Pgmdesc |

## Business Logic

### Start (Event)

```genexus
&AuditTableName = !'EmbarqueDetalle'
	/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	LoadWWPContext.Call(&WWPContext)
	For each EmbarqueDetalle
		where EmbarqueDetalleId = &EmbarqueDetalleId
		Form.Caption = EmbarqueDetalleProducto.ToString()
		&Exists = True
	When none
		Form.Caption = "WWP_RecordNotFound"
		&Exists = False
	Endfor
	If (&Exists)
		&SelectedTabCode = &TabCode
		Tabs.ActivePageControlName = &SelectedTabCode
		Do 'LoadTabs'
	Endif
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### Load (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### Tabs.TabChanged (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	&SelectedTabCode = Tabs.ActivePageControlName
	&LoadAllTabs = false
	Do 'LoadTabs'
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### Rules (Rules)

```genexus

	/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */

parm(in:&EmbarqueDetalleId, in:&TabCode);

	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

