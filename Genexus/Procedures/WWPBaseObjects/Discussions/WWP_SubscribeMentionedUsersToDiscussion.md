...
For Each WWP_NotificationDefinition
	...
	For &WWPUserExtendedId In &MentionWWPUserExtendedIdCollection
		...
		When None
			...
			&WWPSubscription.Notifications.WWPUserExtendedId = &WWPUserExtendedId
			...
			If Not &WWPSubscription.Success()
				WWPBaseObjects.WWP_Logger.Error("Subscribe Mentioned User", &WWPSubscription.GetMessages().ToJson())
			EndIf
		EndFor
	EndFor
When None
	WWPBaseObjects.WWP_Logger.Error(&pgmname, format(!'WWP_NotificationDefinition not found: %1', &WWPNotificationDefinitionName))
EndFor
Commit
```
