# Procedure: ExistenciaWWExport

- **Module:** DB
- **Description:** Existencia WWExport
- **GAM Object:** No
...
Sub 'OpenDocument'
	...
	&Random = Random() * 10000
	&Filename = !"ExistenciaWWExport-" + &Random.ToString().Trim() + !".xlsx"
...
Sub 'WriteFilters'
	...
	If not (&TFExistenciaFechaHora.IsEmpty() AND &TFExistenciaFechaHora_To.IsEmpty())
		...
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Date = &TFExistenciaFechaHora
		WWP_ExportWriteFilter(&ExcelDocument, false, &CellRow, &FirstColumn + 2, "WWP_TSTo")
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 3).Date = &TFExistenciaFechaHora_To
	EndIf
	If not (&TFExistenciaTurnoNombre_Sel.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Turno")
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWP_Export_SecureText(&TFExistenciaTurnoNombre_Sel)
	Else
		If not (&TFExistenciaTurnoNombre.IsEmpty())
			WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Turno")
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWP_Export_SecureText(&TFExistenciaTurnoNombre)
		EndIf
	EndIf
	&CellRow += 2
...
Sub 'WriteData'
	...
	For each DB.Existencia
		...
		For &ColumnsSelector_Column in &ColumnsSelector.Columns
			If &ColumnsSelector_Column.IsVisible = True
				Do Case
					Case &ColumnsSelector_Column.ColumnName = !'ExistenciaFechaHora'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Date = ExistenciaFechaHora
					Case &ColumnsSelector_Column.ColumnName = !'ExistenciaTurnoNombre'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Text = WWP_Export_SecureText(ExistenciaTurnoNombre)
				EndCase
				&VisibleColumnCount += 1
			EndIf
		EndFor		
...
Sub 'InitializeColumnsSelector'
	...
	WWP_ColumnsSelector_Add(&ColumnsSelector, !"ExistenciaFechaHora", '', !'Fecha Hora', True, '')
	WWP_ColumnsSelector_Add(&ColumnsSelector, !"ExistenciaTurnoNombre", '', !'Turno', True, '')
		
	&UserCustomValue = WWPBaseObjects.LoadColumnsSelectorState.Udp(!'DB.ExistenciaWWColumnsSelector')
