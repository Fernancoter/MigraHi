# Procedure: SDExtrusionResultado

- **Module:** Produccion
- **Description:** SDExtrusion Resultado
- **GAM Object:** No
...
| SDTExtrusionResultado | Parameter | GX_SDT | out | SDTExtrusion Resultado |
...
```genexus
	...
	when none
        ...
        &SDTExtrusionResultado = New()
	&SDTExtrusionResultado.FromJson(&ExtrusionResultado.ToJson())
	...
	Sub 'ResumenBobinas'
		...
		for each DB.Bobina
			...
			Do Case 
				Case &BobinaEstado = Produccion.EstadoBobina.Molino
					&BobinasMolino += 1
					&TotalMermaKg += &BobinaMermaKg
				
				Case Not &BobinaEstado in (EstadoBobina.EnProceso, EstadoBobina.EnMedicion, EstadoBobina.Molino)
					&BobinasReposo += 1
					&TotalKg += &BobinaKg
			EndCase
		endfor
...
```genexus
parm(in:&ExtrusionId, out:&SDTExtrusionResultado);
```
