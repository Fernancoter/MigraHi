...
Sub 'WriteFilters'
	...
	Else
		If not (&TFPrensadoProductoNombre.IsEmpty())
			WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Producto")
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFPrensadoProductoNombre)
		EndIf
	EndIf
	...
	Else
		If not (&TFPrensadoOperadorNombre.IsEmpty())
			...
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFPrensadoOperadorNombre)
		EndIf
	EndIf
	...
	If not (&TFPrensadoLevasGradosEntrada.IsEmpty() AND &TFPrensadoLevasGradosEntrada_To.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Levas Grados Entrada")
	...
	If not (&TFPrensadoRodillosKgEntrada.IsEmpty() AND &TFPrensadoRodillosKgEntrada_To.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Rodillos Kg Entrada")
	...
	Else
		If not (&TFPrensadoTroquelNombre.IsEmpty())
			WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Troquel")
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFPrensadoTroquelNombre)
		EndIf
	EndIf
...
Sub 'WriteData'
	...
	For each DB.Prensado
		...
		using Produccion.vwAnaliticaPrensadoDS(&FilterFullText, &TFPrensadoFecha, &TFPrensadoFecha_To, &TFPrensadoPrensaNombre, &TFPrensadoPrensaNombre_Sel, &TFPrensadoTurnoNombre
			...
			If &ColumnsSelector_Column.IsVisible = True
				Do Case
					...
					Case &ColumnsSelector_Column.ColumnName = !'PrensadoProductoNombre'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Text = WWP_Export_SecureText(DB.PrensadoProductoNombre)
		...
		Do Case
			...
			Case DB.PrensadoEstado = EstadoPrensado.Programado
...
Sub 'InitializeColumnsSelector'
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"PrensadoTiempoInterrupcion", '', !'Tiempo Interrupción (min)', True, '')
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"PrensadoLevasKgSalida", '', !'Levas Kg Salida', True, '')
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"PrensadoTroquelNombre", '', !'Troquel', True, '')
	...
	WWP_ColumnsSelector_Add(&ColumnsSelector, !"PrensadoResultadoTotalPalets", '', !'Pallets', True, '')
