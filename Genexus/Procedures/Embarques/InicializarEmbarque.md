...
if(&EmbarqueId = 0)
	...
	for each DB.Remission
		...
		&EmbarqueDetalle.EmbarqueDetalleProducto = DB.ProductNumber
		...
		if(Not &DeliveryDate.IsEmpty())
			...
			if(&DeliveryDate <> &OrderDeliveryDate)
				...
				if(&Embarque.Success())
					...
					//Enviar notificación
					WWPBaseObjects.Notifications.Common.WWP_SendNotification(!'CambioFechaEstimada', !"Embarque", '', "fas fa-info", "Cambio Fecha de Embarque", &Embarque.EmbarqueCodigo.ToString(), "El embarque " + &Embarque.EmbarqueCodigo.ToString() + " ha cambiado su fecha estimada de entrega", '', '', '', true)
				endif
			endif
		endif
	endif
