# Procedure: RemissionsWWExport

- **Module:** Embarques
- **Description:** Remissions WWExport
- **GAM Object:** No
...
Sub 'OpenDocument'
	...
	&Random = Random() * 10000
	&Filename = !"RemissionsWWExport-" + &Random.ToString().Trim() + !".xlsx"
...
Sub 'WriteFilters'
	...
	If not (&TFOrderDoc_Sel.IsEmpty())
		...
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFOrderDoc_Sel)
	Else
	...
	If not (&TFCustomer_Sel.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Código Cliente")
	...
	If not (&TFRemissionStatus_Sel.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Estatus SAE")
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWP_Export_SecureText(&TFRemissionStatus_Sel)
	Else
		If not (&TFRemissionStatus.IsEmpty())
			WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Estatus SAE")
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWP_Export_SecureText(&TFRemissionStatus)
		EndIf
	EndIf
	...
	Else
		If not (&TFFTBCustomer.IsEmpty())
			WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Cliente FTB")
...
Sub 'WriteData'
	...
	For each DB.Remission
		...
		order (WWPBaseObjects.RemissionPXS)  when &OrderedBy = 13 AND &OrderedDsc = True
...
Sub 'InitializeColumnsSelector'
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"RemissionDoc", '', !'Remisión SAE', True, '')
	...
	&UserCustomValue = LoadColumnsSelectorState.Udp(!'Embarques.RemissionsWWColumnsSelector')
