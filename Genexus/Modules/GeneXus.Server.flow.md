# Flujo del módulo: GeneXus.Server

> Generado por `Scripts/generate-module-flows.ps1` a partir de `_index.json` + `_menu.json` + `_processes.json` + `Modules/GeneXus.Server.md`.
...
_Módulo de infraestructura sin flujo propio_: sus objetos se invocan como utilidades desde **1 módulos externos** (1 llamadas entrantes). No se emite diagrama de flujo — ver la sección de llamadas cross-module para el uso real.
...
|---|---:|---|
| `WWPBaseObjects.Notifications.Web` | 1 | `WWP_SendWebNotification` |
...
- **Propósito:** Módulo con 1 objetos parseados. Sin entry points en `_menu.json` -- accedido indirectamente desde: WWPBaseObjects.Notifications.Web.
