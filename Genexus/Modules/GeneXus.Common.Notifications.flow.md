# Flujo del módulo: GeneXus.Common.Notifications

> Generado por `Scripts/generate-module-flows.ps1` a partir de `_index.json` + `_menu.json` + `_processes.json` + `Modules/GeneXus.Common.Notifications.md`.
...
|---|---:|---|
| `GeneXus.Common` | 1 | `DirectionsRequestParameters` |
...
- **Propósito:** Módulo con 7 objetos parseados. Sin entry points en `_menu.json` -- accedido indirectamente desde: GeneXus.Common.
