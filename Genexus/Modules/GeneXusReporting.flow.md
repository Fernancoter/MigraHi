# Flujo del módulo: GeneXusReporting

> Generado por `Scripts/generate-module-flows.ps1` a partir de `_index.json` + `_menu.json` + `_processes.json` + `Modules/GeneXusReporting.md`.
...
| Módulos que LO LLAMAN | 5 (104 calls) |
...
_Módulo de infraestructura sin flujo propio_: sus objetos se invocan como utilidades desde **5 módulos externos** (104 llamadas entrantes). No se emite diagrama de flujo — ver la sección de llamadas cross-module para el uso real.
...
|---|---:|---|
| `Produccion` | 64 | `InicioPrensado`, `InicioProduccion`, `TurnosPorSemanaExtrusoras` |
| `Web` | 16 | `ModuleB`, `ModuleA` |
| `Seguridad` | 8 | `inicioSeguridad` |
| `SAE` | 8 | `ReportITW` |
| `Embarques` | 8 | `InicioEmbarques` |
...
- **Propósito:** Módulo con 13 objetos parseados. Sin entry points en `_menu.json` -- accedido indirectamente desde: Embarques, Produccion, SAE.
