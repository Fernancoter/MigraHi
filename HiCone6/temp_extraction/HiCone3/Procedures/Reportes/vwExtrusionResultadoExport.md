...
Sub 'WriteFilters'
	...
	If not (&TFExtrusionResultadoBobinasReposo.IsEmpty() AND &TFExtrusionResultadoBobinasReposo_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Bobinas Reposo")
	...
	If not (&TFExtrusionResultadoVelLaminadora.IsEmpty() AND &TFExtrusionResultadoVelLaminadora_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Vel Laminadora")
	...
	If not (&TFExtrusionResultadoVelHusillo.IsEmpty() AND &TFExtrusionResultadoVelHusillo_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Vel Husillo")
	...
	If not (&TFExtrusionResultadoTotalKg.IsEmpty() AND &TFExtrusionResultadoTotalKg_To.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Total Kg")
	...
	If not (&TFExtrusionRevHusilloMolido.IsEmpty() AND &TFExtrusionRevHusilloMolido_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Extrusion Rev Husillo Molido")
	...
	If not (&TFExtrusionVirgenKg.IsEmpty() AND &TFExtrusionVirgenKg_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Extrusion Virgen Kg")
	...
	If not (&TFExtrusionMolidoKg.IsEmpty() AND &TFExtrusionMolidoKg_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Extrusion Molido Kg")
...
Sub 'WriteData'
	...
	For each DB.ExtrusionResultado
		...
		order (ExtrusionResultadoBobinasReposo)  when &OrderedBy = 7 AND &OrderedDsc = True
			...
			If &ColumnsSelector_Column.IsVisible = True
				Do Case
					...
					Case &ColumnsSelector_Column.ColumnName = !'ExtrusionOperadorNombre'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Text = WWP_Export_SecureText(Produccion.ExtrusionOperadorNombre)
					...
					Case &ColumnsSelector_Column.ColumnName = !'ExtrusionResultadoTotalMermaKg'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Number = ExtrusionResultadoTotalMermaKg
...
Sub 'InitializeColumnsSelector'
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"ExtrusionExtrusoraNombre", '', !'Extrusora', True, '')
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"ExtrusionResultadoObservaciones", '', !'Observaciones', False, '')
...
Sub 'LoadGridState'
	...
	If &Session.Get(!"Reportes.vwExtrusionResultadoGridState") = !""
		&GridState.FromXml(WWPBaseObjects.LoadGridState.Udp(!"Reportes.vwExtrusionResultadoGridState"))
	Else
