...
Sub 'WriteFilters'
	...
	If not (&TFRemissionDoc_Sel.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Remisión SAE")
	...
	If not (&TFRemissionStatus_Sel.IsEmpty())
		...
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFRemissionStatus_Sel)
	Else
		If not (&TFRemissionStatus.IsEmpty())
			WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Estatus SAE")
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFRemissionStatus)
		EndIf
	EndIf
	...
	If not (&TFFTBCustomer_Sel.IsEmpty())
		...
		&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFFTBCustomer_Sel)
	Else
...
Sub 'WriteData'
	...
	For each DB.Remission
		...
		order (DB.RemissionTotal)  when &OrderedBy = 15 AND &OrderedDsc = True
...
Sub 'InitializeColumnsSelector'
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"OrderDoc", '', !'Pedido SAE', True, '')
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"RemissionPrice", '', !'Precio', False, '')
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"FTBCustomer", '', !'Cliente FTB', False, '')
