...
Sub 'WriteFilters'
	...
	If not (&TFCarreraFechaValidacion.IsEmpty() AND &TFCarreraFechaValidacion_To.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Fecha Validación")
	...
	If not (&TFPaletHoraInicioEnsamble.IsEmpty() AND &TFPaletHoraInicioEnsamble_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Pallet H. Ini. Ensamble")
	...
	If not (&TFCarreteNoSerie_Sel.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Carrete No Serie")
	...
	If not (&TFInicioPrensadoBobinaNoSerie_Sel.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Bobina")
	...
	If not (&TFInicioPrensadoBobinaNo.IsEmpty() AND &TFInicioPrensadoBobinaNo_To.IsEmpty())
		WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "N° de Bobina")
...
Sub 'WriteData'
	...
	For each DB.PaletCarrete
		...
		order (DB.InicioPBPrensaNombre)  when &OrderedBy = 2 AND &OrderedDsc = True
		...
		order WWPBaseObjects.PrensadoOperadorNombre  when &OrderedBy = 13 AND &OrderedDsc = False
		...
		order Common.InicioPrensadoBobinaReposoHoras  when &OrderedBy = 18 AND &OrderedDsc = False
		...
		order WWPBaseObjects.ExtrusionHoraIniciaProceso  when &OrderedBy = 21 AND &OrderedDsc = False
		...
		order (Notifications.ExtrusionHoraFinProceso)  when &OrderedBy = 22 AND &OrderedDsc = True
			...
			If &ColumnsSelector_Column.IsVisible = True
				Do Case
					...
					Case &ColumnsSelector_Column.ColumnName = !'PaletHoraFinEnsamble'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Date = WWPBaseObjects.PaletHoraFinEnsamble
					Case &ColumnsSelector_Column.ColumnName = !'PrensadoOperadorNombre'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Text = WWPBaseObjects.WWP_Export_SecureText(PrensadoOperadorNombre)
...
Sub 'InitializeColumnsSelector'
	...
	WWP_ColumnsSelector_Add(&ColumnsSelector, !"ExtrusionHoraIniciaProceso", '', !'Hr. Ini. Extrusión', True, '')
