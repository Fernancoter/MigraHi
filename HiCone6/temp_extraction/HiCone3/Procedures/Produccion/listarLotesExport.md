...
Sub 'WriteFilters'
	...
	Else
		If not (&TFLoteEmbarque.IsEmpty())
			WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Lote NO.")
	...
	Else
		If not (&TFLotePO.IsEmpty())
			WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Lote PO.")
			&ExcelDocument.Cells(&CellRow, &FirstColumn + 1).Text = WWPBaseObjects.WWP_Export_SecureText(&TFLotePO)
		EndIf
	EndIf
	...
	Else
		If not (&TFLoteTrunkNo.IsEmpty())
			WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Trunk No")
	...
	If not (&TFLoteSiloNombre_Sel.IsEmpty())
		WWPBaseObjects.WWP_ExportWriteFilter(&ExcelDocument, true, &CellRow, &FirstColumn, "Silo")
...
Sub 'WriteData'
	...
	For each DB.Lote
		...
		order (DB.LoteSiloNombre)  when &OrderedBy = 6 AND &OrderedDsc = True
			...
			If &ColumnsSelector_Column.IsVisible = True
				Do Case
					Case &ColumnsSelector_Column.ColumnName = !'LoteEmbarque'
						&ExcelDocument.Cells(&CellRow, &FirstColumn + &VisibleColumnCount).Text = WWPBaseObjects.WWP_Export_SecureText(LoteEmbarque)
...
Sub 'InitializeColumnsSelector'
	...
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"LoteSiloEnumTipoMaterial", '', !'Estado Material', True, '')
	WWP_ColumnsSelector_Add(&ColumnsSelector, !"LoteKg", '', !'Kg', True, '')
	WWPBaseObjects.WWP_ColumnsSelector_Add(&ColumnsSelector, !"LoteConsumido", '', !'Consumido', True, '')
