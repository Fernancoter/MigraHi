# WebPanel: vwAnaliticaBobina

- **Module:** Produccion
- **Description:**  Bobina
- **GAM Object:** No

## Data Dictionary / Parameters

| Name | Element Type | Data Type | Accessor | Description |
|---|---|---|---|---|
| IsAuthorized | Variable | Boolean |  | Is Authorized |
| WWPContext | Variable | GX_SDT |  | WWPContext |
| SecurityFunctionalityKeys | Variable | VARCHAR |  | Security Functionality Keys |
| HTTPRequest | Variable | GX_USRDEFTYP |  | HTTPRequest |
| TrnContext | Variable | GX_SDT |  | Trn Context |
| TrnContextAtt | Variable | GX_SDT |  | Trn Context Att |
| GridState | Variable | GX_SDT |  | Grid State |
| GridStateFilterValue | Variable | GX_SDT |  | Grid State Filter Value |
| OrderedBy | Variable | NUMERIC |  | Ordered By |
| OrderedDsc | Variable | Boolean |  | Ordered Dsc |
| OrderedByAux | Variable | NUMERIC |  | Ordered By Aux |
| FilterFullText | Variable | VARCHAR |  | Filter Full Text |
| ExcelFilename | Variable | VARCHAR |  | Excel Filename |
| ErrorMessage | Variable | VARCHAR |  | Error Message |
| ColumnsSelectorXML | Variable | LONGVARCHAR |  | Columns Selector XML |
| UserCustomValue | Variable | LONGVARCHAR |  | User Custom Value |
| ColumnsSelector | Variable | GX_SDT |  | Columns Selector |
| ColumnsSelectorAux | Variable | GX_SDT |  | Columns Selector Aux |
| Session | Variable | GX_USRDEFTYP |  | Session |
| ManageFiltersData | Variable | GX_SDT |  | Manage Filters Data |
| ManageFiltersXml | Variable | LONGVARCHAR |  | Manage Filters Xml |
| ManageFiltersExecutionStep | Variable | NUMERIC |  | Manage Filters Execution Step |
| TFBobinaId | Variable | NUMERIC |  | TFBobina Id |
| TFBobinaId_To | Variable | NUMERIC |  | TFBobina Id_To |
| TFExtrusionId | Variable | NUMERIC |  | TFExtrusion Id |
| TFExtrusionId_To | Variable | NUMERIC |  | TFExtrusion Id_To |
| TFExtrusionTurnoNombre | Variable | VARCHAR |  | TFExtrusion Turno Nombre |
| TFExtrusionTurnoNombre_Sel | Variable | VARCHAR |  | TFExtrusion Turno Nombre_Sel |
| TFExtrusionOperadorNombre | Variable | VARCHAR |  | TFExtrusion Operador Nombre |
| TFExtrusionOperadorNombre_Sel | Variable | VARCHAR |  | TFExtrusion Operador Nombre_Sel |
| TFExtrusionExtrusoraNombre | Variable | VARCHAR |  | TFExtrusion Extrusora Nombre |
| TFExtrusionExtrusoraNombre_Sel | Variable | VARCHAR |  | TFExtrusion Extrusora Nombre_Sel |
| TFBobinaNoSerie | Variable | VARCHAR |  | TFBobina No Serie |
| TFBobinaNoSerie_Sel | Variable | VARCHAR |  | TFBobina No Serie_Sel |
| TFBobinaOrigen_SelsJson | Variable | LONGVARCHAR |  | TFBobina Origen_Sels Json |
| TFBobinaOrigen_Sels | Variable | VARCHAR |  | TFBobina Origen_Sels |
| TFBobinaHoraInicio | Variable | DATETIME |  | TFBobina Hora Inicio |
| TFBobinaHoraInicio_To | Variable | DATETIME |  | TFBobina Hora Inicio_To |
| DDO_BobinaHoraInicioAuxDate | Variable | DATE |  | DDO_Bobina Hora Inicio Aux Date |
| DDO_BobinaHoraInicioAuxDateTo | Variable | DATE |  | DDO_Bobina Hora Inicio Aux Date To |
| DDO_BobinaHoraInicioAuxDateText | Variable | VARCHAR |  | DDO_Bobina Hora Inicio Aux Date Text |
| TFBobinaHoraSalida | Variable | DATETIME |  | TFBobina Hora Salida |
| TFBobinaHoraSalida_To | Variable | DATETIME |  | TFBobina Hora Salida_To |
| DDO_BobinaHoraSalidaAuxDate | Variable | DATE |  | DDO_Bobina Hora Salida Aux Date |
| DDO_BobinaHoraSalidaAuxDateTo | Variable | DATE |  | DDO_Bobina Hora Salida Aux Date To |
| DDO_BobinaHoraSalidaAuxDateText | Variable | VARCHAR |  | DDO_Bobina Hora Salida Aux Date Text |
| TFBobinaNo | Variable | NUMERIC |  | TFBobina No |
| TFBobinaNo_To | Variable | NUMERIC |  | TFBobina No_To |
| TFBobinaKg | Variable | NUMERIC |  | TFBobina Kg |
| TFBobinaKg_To | Variable | NUMERIC |  | TFBobina Kg_To |
| TFBobinaMermaKg | Variable | NUMERIC |  | TFBobina Merma Kg |
| TFBobinaMermaKg_To | Variable | NUMERIC |  | TFBobina Merma Kg_To |
| TFBobinaEspesor | Variable | NUMERIC |  | TFBobina Espesor |
| TFBobinaEspesor_To | Variable | NUMERIC |  | TFBobina Espesor_To |
| TFBobinaObservaciones | Variable | VARCHAR |  | TFBobina Observaciones |
| TFBobinaObservaciones_Sel | Variable | VARCHAR |  | TFBobina Observaciones_Sel |
| TFBobinaRechazadaObservaciones | Variable | VARCHAR |  | TFBobina Rechazada Observaciones |
| TFBobinaRechazadaObservaciones_Sel | Variable | VARCHAR |  | TFBobina Rechazada Observaciones_Sel |
| TFBobinaEstado_SelsJson | Variable | LONGVARCHAR |  | TFBobina Estado_Sels Json |
| TFBobinaEstado_Sels | Variable | VARCHAR |  | TFBobina Estado_Sels |
| TFBobinaCarreras | Variable | NUMERIC |  | TFBobina Carreras |
| TFBobinaCarreras_To | Variable | NUMERIC |  | TFBobina Carreras_To |
| TFBobinaIniciaReposo | Variable | DATETIME |  | TFBobina Inicia Reposo |
| TFBobinaIniciaReposo_To | Variable | DATETIME |  | TFBobina Inicia Reposo_To |
| DDO_BobinaIniciaReposoAuxDate | Variable | DATE |  | DDO_Bobina Inicia Reposo Aux Date |
| DDO_BobinaIniciaReposoAuxDateTo | Variable | DATE |  | DDO_Bobina Inicia Reposo Aux Date To |
| DDO_BobinaIniciaReposoAuxDateText | Variable | VARCHAR |  | DDO_Bobina Inicia Reposo Aux Date Text |
| TFBobinaMinutosEnReposo | Variable | NUMERIC |  | TFBobina Minutos En Reposo |
| TFBobinaMinutosEnReposo_To | Variable | NUMERIC |  | TFBobina Minutos En Reposo_To |
| TFBobinaMotivoMolino_SelsJson | Variable | LONGVARCHAR |  | TFBobina Motivo Molino_Sels Json |
| TFBobinaMotivoMolino_Sels | Variable | VARCHAR |  | TFBobina Motivo Molino_Sels |
| TFBobinaSiloMolidoId | Variable | NUMERIC |  | TFBobina Silo Molido Id |
| TFBobinaSiloMolidoId_To | Variable | NUMERIC |  | TFBobina Silo Molido Id_To |
| TFBobinaSiloMolidoNombre | Variable | VARCHAR |  | TFBobina Silo Molido Nombre |
| TFBobinaSiloMolidoNombre_Sel | Variable | VARCHAR |  | TFBobina Silo Molido Nombre_Sel |
| TFBobinaSiloVirgenId | Variable | NUMERIC |  | TFBobina Silo Virgen Id |
| TFBobinaSiloVirgenId_To | Variable | NUMERIC |  | TFBobina Silo Virgen Id_To |
| TFBobinaSiloVirgenNombre | Variable | VARCHAR |  | TFBobina Silo Virgen Nombre |
| TFBobinaSiloVirgenNombre_Sel | Variable | VARCHAR |  | TFBobina Silo Virgen Nombre_Sel |
| TFBobinaLoteVirgen | Variable | CHARACTER |  | TFBobina Lote Virgen |
| TFBobinaLoteVirgen_Sel | Variable | CHARACTER |  | TFBobina Lote Virgen_Sel |
| TFBobinaProductoId | Variable | NUMERIC |  | TFBobina Producto Id |
| TFBobinaProductoId_To | Variable | NUMERIC |  | TFBobina Producto Id_To |
| TFBobinaProductoNombre | Variable | VARCHAR |  | TFBobina Producto Nombre |
| TFBobinaProductoNombre_Sel | Variable | VARCHAR |  | TFBobina Producto Nombre_Sel |
| TFBobinaProductoTipoMaterial_SelsJson | Variable | LONGVARCHAR |  | TFBobina Producto Tipo Material_Sels Json |
| TFBobinaProductoTipoMaterial_Sels | Variable | VARCHAR |  | TFBobina Producto Tipo Material_Sels |
| TFBobinaPrensadoPrensaNombre | Variable | VARCHAR |  | TFBobina Prensado Prensa Nombre |
| TFBobinaPrensadoPrensaNombre_Sel | Variable | VARCHAR |  | TFBobina Prensado Prensa Nombre_Sel |
| TFBobinaReposoEnHoras | Variable | NUMERIC |  | TFBobina Reposo En Horas |
| TFBobinaReposoEnHoras_To | Variable | NUMERIC |  | TFBobina Reposo En Horas_To |
| DDO_TitleSettingsIcons | Variable | GX_SDT |  | DDO_Title Settings Icons |
| GAMSession | Variable | GX_EXTERNAL_OBJECT |  | GAMSession |
| GAMErrors | Variable | GX_EXTERNAL_OBJECT |  | GAMErrors |
| PageToGo | Variable | NUMERIC |  | Page To Go |
| GridCurrentPage | Variable | NUMERIC |  | Grid Current Page |
| GridPageCount | Variable | NUMERIC |  | Grid Page Count |
| GridAppliedFilters | Variable | VARCHAR |  | Grid Applied Filters |
| Update | Variable | CHARACTER |  | Update |
| IsAuthorized_Update | Variable | Boolean |  | Is Authorized_Update |
| Delete | Variable | CHARACTER |  | Delete |
| IsAuthorized_Delete | Variable | Boolean |  | Is Authorized_Delete |
| AGExportData | Variable | GX_SDT |  | AGExport Data |
| AGExportDataItem | Variable | GX_SDT |  | AGExport Data Item |
| IsAuthorized_Insert | Variable | Boolean |  | Is Authorized_Insert |
| DetalleBobina_Load | Variable | NUMERIC |  | Detalle Bobina_Load |
| GridConditionalFormattingFilter | Variable | NUMERIC |  | Grid Conditional Formatting Filter |
| GridBadge | Variable | VARCHAR |  | Grid Badge |
| Opciones | Variable | NUMERIC |  | Opciones |
| IsAuthorized_Display | Variable | Boolean |  | Is Authorized_Display |
| DetalleBobina | Variable | CHARACTER |  | Detalle Bobina |
| TotBobinaKg | Variable | NUMERIC |  | Tot Bobina Kg |
| TotValueBobinaKg | Variable | VARCHAR |  | Tot Value Bobina Kg |
| TotBobinaNo | Variable | NUMERIC |  | Tot Bobina No |
| TotValueBobinaNo | Variable | VARCHAR |  | Tot Value Bobina No |
| PDFNombre | Variable | VARCHAR |  | PDFNombre |
| Window | Variable | GX_USRDEFTYP |  | Window |
| Url | Variable | VARCHAR |  | Url |
| AuditTableName | Variable | VARCHAR |  | Audit Table Name |
| IsAuthorized_deletedRecords | Variable | Boolean |  | Is Authorized_deleted Records |
| TabCode | Variable | CHARACTER |  | Tab Code |
| NowDate | Variable | DATE |  | Now Date |
| WebSession | Variable | GX_USRDEFTYP |  | Web Session |
| IsAuthorized_cmdImprimir | Variable | Boolean |  | Is Authorized_cmd Imprimir |
| BobinaId_WSKey | Variable | VARCHAR |  | Bobina Id_WSKey |
| BobinaNoSerie_WSKey | Variable | VARCHAR |  | Bobina No Serie_WSKey |
| TFBobinaDispersion | Variable | NUMERIC |  | TFBobina Dispersion |
| TFBobinaDispersion_To | Variable | NUMERIC |  | TFBobina Dispersion_To |
| TFBobinaDesviacionEstandar | Variable | NUMERIC |  | TFBobina Desviacion Estandar |
| TFBobinaDesviacionEstandar_To | Variable | NUMERIC |  | TFBobina Desviacion Estandar_To |
| IsAuthorized_BobinaNoSerie | Variable | Boolean |  | Is Authorized_Bobina No Serie |
| IsAuthorized_ExtrusionExtrusoraNombre | Variable | Boolean |  | Is Authorized_Extrusion Extrusora Nombre |
| TFExtrusionLotePaqueteAditivos | Variable | VARCHAR |  | TFExtrusion Lote Paquete Aditivos |
| TFExtrusionLotePaqueteAditivos_Sel | Variable | VARCHAR |  | TFExtrusion Lote Paquete Aditivos_Sel |
| TFBobinaInterrupcionId | Variable | NUMERIC |  | TFBobina Interrupcion Id |
| TFBobinaInterrupcionId_To | Variable | NUMERIC |  | TFBobina Interrupcion Id_To |
| TFInterrupcionTiempo | Variable | NUMERIC |  | TFInterrupcion Tiempo |
| TFInterrupcionTiempo_To | Variable | NUMERIC |  | TFInterrupcion Tiempo_To |
| TFInterrupcionMotivo | Variable | VARCHAR |  | TFInterrupcion Motivo |
| TFInterrupcionMotivo_Sel | Variable | VARCHAR |  | TFInterrupcion Motivo_Sel |
| IsAuthorized_InterrupcionMotivo | Variable | Boolean |  | Is Authorized_Interrupcion Motivo |
| TFBobinaInterrupcionesId | Variable | NUMERIC |  | TFBobina Interrupciones Id |
| TFBobinaInterrupcionesId_To | Variable | NUMERIC |  | TFBobina Interrupciones Id_To |
| TFBobinaInterrupcionesMotivo | Variable | VARCHAR |  | TFBobina Interrupciones Motivo |
| TFBobinaInterrupcionesMotivo_Sel | Variable | VARCHAR |  | TFBobina Interrupciones Motivo_Sel |
| TFBobinaInterrupcionesTiempo | Variable | NUMERIC |  | TFBobina Interrupciones Tiempo |
| TFBobinaInterrupcionesTiempo_To | Variable | NUMERIC |  | TFBobina Interrupciones Tiempo_To |
| TFBobinaDownTimeCode | Variable | VARCHAR |  | TFBobina Down Time Code |
| TFBobinaDownTimeCode_Sel | Variable | VARCHAR |  | TFBobina Down Time Code_Sel |
| TFBobinaDownTimeDescription | Variable | VARCHAR |  | TFBobina Down Time Description |
| TFBobinaDownTimeDescription_Sel | Variable | VARCHAR |  | TFBobina Down Time Description_Sel |
| IsAuthorized_BobinaInterrupcionesMotivo | Variable | Boolean |  | Is Authorized_Bobina Interrupciones Motivo |
| TFExtrusionExtrusoraColorEstacion_SelsJson | Variable | LONGVARCHAR |  | TFExtrusion Extrusora Color Estacion_Sels Json |
| TFExtrusionExtrusoraColorEstacion_Sels | Variable | VARCHAR |  | TFExtrusion Extrusora Color Estacion_Sels |
| TFBobinaColorEstacion_SelsJson | Variable | LONGVARCHAR |  | TFBobina Color Estacion_Sels Json |
| TFBobinaColorEstacion_Sels | Variable | VARCHAR |  | TFBobina Color Estacion_Sels |
| Today | Variable | DATE |  | Today |
| Time | Variable | CHARACTER |  | Time |
| Pgmname | Variable | CHARACTER |  | Pgmname |
| Pgmdesc | Variable | CHARACTER |  | Pgmdesc |

## Business Logic

### Start (Event)

```genexus
&BobinaId_WSKey = ""
	&BobinaNoSerie_WSKey = !"Print"
	
	&NowDate.FromString(&WebSession.Get(!"FechaInicialConsulta"))
		
	&AuditTableName = !'Bobina'
	ReposoTranscurrido.Call()
	/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	TFBobinaHoraSalida_RangePicker.Attach(&DDO_BobinaHoraSalidaAuxDateText.InternalName)
	TFBobinaHoraInicio_RangePicker.Attach(&DDO_BobinaHoraInicioAuxDateText.InternalName)
	
	Grid.Rows = Page.Rows
	Grid_Empowerer.GridInternalName = Grid.InternalName
	DDO_GridColumnsSelector.GridInternalName = Grid.InternalName
	ddo_AGExport.TitleControlIdToReplace = BtnAGExport.InternalName
	&AGExportData = new()
	
	&AGExportDataItem = new()
	&AGExportDataItem.Title = ''
	&AGExportDataItem.Icon = ActionExport.Link()
	&AGExportDataItem.EventKey = !'Export'
	&AGExportDataItem.IsDivider = False
	&AGExportData.Add(&AGExportDataItem)
	
	&AGExportDataItem = new()
	&AGExportDataItem.Title = ''
	&AGExportDataItem.Icon = ActionExportReport.Link()
	&AGExportDataItem.EventKey = !'ExportReport'
	&AGExportDataItem.IsDivider = False
	&AGExportData.Add(&AGExportDataItem)
	
	DDC_Subscriptions.TitleControlIdToReplace = BtnSubscriptions.InternalName
	
	&IsAuthorized_BobinaNoSerie = SecGAMIsAuthByFunctionalityKey.Udp(!'bobinaview_Execute')
	
	&IsAuthorized_ExtrusionExtrusoraNombre = SecGAMIsAuthByFunctionalityKey.Udp(!'extrusionview_Execute')
	
	&IsAuthorized_BobinaInterrupcionesMotivo = SecGAMIsAuthByFunctionalityKey.Udp(!'interrupcionview_Execute')
	
	&GAMSession = GAMSession.Get(&GAMErrors)
	DDO_Grid.GridInternalName = Grid.InternalName
	DDO_Grid.GAMOAuthToken = &GAMSession.Token
	Form.Caption = ' Bobina'
	Do 'PrepareTransaction'
	Do 'LoadGridState'
	If &OrderedBy < 1
		&OrderedBy = 1
		Do 'SetDDOSortedStatus'
	EndIf
	If &HTTPRequest.Method = HttpMethod.Get
		Do 'LoadSavedFilters'
	EndIf
	&DDO_TitleSettingsIcons = GetWWPTitleSettingsIcons()
	DDO_GridColumnsSelector.TitleControlIdToReplace = BtnEditColumns.InternalName
	GridPaginationBar.RowsPerPageSelectedValue = Grid.Rows
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### Refresh (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	LoadWWPContext.Call(&WWPContext)
	Do 'CheckSecurityForActions'
	Do Case
		Case &ManageFiltersExecutionStep = 1
			&ManageFiltersExecutionStep = 2
		Case &ManageFiltersExecutionStep = 2
			&ManageFiltersExecutionStep = 0
			Do 'LoadSavedFilters'
	EndCase
	Do 'SaveGridState'
	
	If &Session.Get(!'Produccion.vwAnaliticaBobinaColumnsSelector') <> ''
		&ColumnsSelectorXML = &Session.Get(!'Produccion.vwAnaliticaBobinaColumnsSelector')
		&ColumnsSelector.FromXml(&ColumnsSelectorXML)
	Else
		Do 'InitializeColumnsSelector'
	EndIf
	BobinaId.Visible = &ColumnsSelector.Columns.Item(1).IsVisible
	BobinaNoSerie.Visible = &ColumnsSelector.Columns.Item(2).IsVisible
	ExtrusionId.Visible = &ColumnsSelector.Columns.Item(3).IsVisible
	ExtrusionExtrusoraNombre.Visible = &ColumnsSelector.Columns.Item(4).IsVisible
	ExtrusionTurnoNombre.Visible = &ColumnsSelector.Columns.Item(5).IsVisible
	BobinaColorEstacion.Visible = &ColumnsSelector.Columns.Item(6).IsVisible
	BobinaOrigen.Visible = &ColumnsSelector.Columns.Item(7).IsVisible
	BobinaEstado.Visible = &ColumnsSelector.Columns.Item(8).IsVisible
	BobinaHoraInicio.Visible = &ColumnsSelector.Columns.Item(9).IsVisible
	BobinaHoraSalida.Visible = &ColumnsSelector.Columns.Item(10).IsVisible
	BobinaDesviacionEstandar.Visible = &ColumnsSelector.Columns.Item(11).IsVisible
	BobinaKg.Visible = &ColumnsSelector.Columns.Item(12).IsVisible
	BobinaMermaKg.Visible = &ColumnsSelector.Columns.Item(13).IsVisible
	BobinaNo.Visible = &ColumnsSelector.Columns.Item(14).IsVisible
	BobinaReposoEnHoras.Visible = &ColumnsSelector.Columns.Item(15).IsVisible
	ExtrusionOperadorNombre.Visible = &ColumnsSelector.Columns.Item(16).IsVisible
	BobinaObservaciones.Visible = &ColumnsSelector.Columns.Item(17).IsVisible
	BobinaRechazadaObservaciones.Visible = &ColumnsSelector.Columns.Item(18).IsVisible
	BobinaSiloMolidoNombre.Visible = &ColumnsSelector.Columns.Item(19).IsVisible
	BobinaSiloVirgenNombre.Visible = &ColumnsSelector.Columns.Item(20).IsVisible
	BobinaLoteVirgen.Visible = &ColumnsSelector.Columns.Item(21).IsVisible
	ExtrusionLotePaqueteAditivos.Visible = &ColumnsSelector.Columns.Item(22).IsVisible
	BobinaProductoId.Visible = &ColumnsSelector.Columns.Item(23).IsVisible
	BobinaProductoNombre.Visible = &ColumnsSelector.Columns.Item(24).IsVisible
	BobinaProductoTipoMaterial.Visible = &ColumnsSelector.Columns.Item(25).IsVisible
	BobinaPrensadoPrensaNombre.Visible = &ColumnsSelector.Columns.Item(26).IsVisible
	BobinaInterrupcionesMotivo.Visible = &ColumnsSelector.Columns.Item(27).IsVisible
	BobinaInterrupcionesTiempo.Visible = &ColumnsSelector.Columns.Item(28).IsVisible
	BobinaDownTimeCode.Visible = &ColumnsSelector.Columns.Item(29).IsVisible
	BobinaDownTimeDescription.Visible = &ColumnsSelector.Columns.Item(30).IsVisible
	Do 'InitializeTotalizers'
	&GridCurrentPage = Grid.CurrentPage
	&GridPageCount = Grid.PageCount
	&GridAppliedFilters = WWP_GetAppliedFiltersDescription(&Pgmname)
	Do 'CalculateTotalizers'
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### GridPaginationBar.ChangePage (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	Do Case
		Case GridPaginationBar.SelectedPage = !'Previous'
			Grid.PreviousPage()
		Case GridPaginationBar.SelectedPage = !'Next'
			Grid.NextPage()
		Otherwise
			&PageToGo.FromString(GridPaginationBar.SelectedPage)
			Grid.GotoPage(&PageToGo)
	EndCase
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### GridPaginationBar.ChangeRowsPerPage (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	Grid.Rows = GridPaginationBar.RowsPerPageSelectedValue
	Grid.FirstPage()
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### DDO_Grid.OnOptionClicked (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	Do Case
		Case DDO_Grid.ActiveEventKey = !'<#OrderASC#>' OR DDO_Grid.ActiveEventKey = !'<#OrderDSC#>'
			&OrderedBy.FromString(DDO_Grid.SelectedValue_get)
			&OrderedDsc = iif(DDO_Grid.ActiveEventKey = !'<#OrderDSC#>', true, false)
			Do 'SetDDOSortedStatus'
			Grid.FirstPage()
		Case DDO_Grid.ActiveEventKey = !'<#Filter#>'
			Do Case
				Case DDO_Grid.SelectedColumn = !'TagColumn'
					&GridConditionalFormattingFilter.FromString(DDO_Grid.SelectedColumnFixedFilter)
				Case DDO_Grid.SelectedColumn = !'BobinaId'
					&TFBobinaId.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaId_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaNoSerie'
					&TFBobinaNoSerie = DDO_Grid.FilteredText_get
					&TFBobinaNoSerie_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'ExtrusionId'
					&TFExtrusionId.FromString(DDO_Grid.FilteredText_get)
					&TFExtrusionId_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'ExtrusionExtrusoraNombre'
					&TFExtrusionExtrusoraNombre = DDO_Grid.FilteredText_get
					&TFExtrusionExtrusoraNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'ExtrusionTurnoNombre'
					&TFExtrusionTurnoNombre = DDO_Grid.FilteredText_get
					&TFExtrusionTurnoNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaColorEstacion'
					&TFBobinaColorEstacion_SelsJson = DDO_Grid.SelectedValue_get
					&TFBobinaColorEstacion_Sels.FromJson(&TFBobinaColorEstacion_SelsJson)
				Case DDO_Grid.SelectedColumn = !'BobinaOrigen'
					&TFBobinaOrigen_SelsJson = DDO_Grid.SelectedValue_get
					&TFBobinaOrigen_Sels.FromJson(&TFBobinaOrigen_SelsJson)
				Case DDO_Grid.SelectedColumn = !'BobinaEstado'
					&TFBobinaEstado_SelsJson = DDO_Grid.SelectedValue_get
					&TFBobinaEstado_Sels.FromJson(&TFBobinaEstado_SelsJson)
				Case DDO_Grid.SelectedColumn = !'BobinaHoraInicio'
					&TFBobinaHoraInicio.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaHoraInicio_To.FromString(DDO_Grid.FilteredTextTo_get)
					If Not &TFBobinaHoraInicio_To.IsEmpty()
						&TFBobinaHoraInicio_To.Set(&TFBobinaHoraInicio_To.Year(), &TFBobinaHoraInicio_To.Month(), &TFBobinaHoraInicio_To.Day(), 23, 59, 59)
					EndIf
				Case DDO_Grid.SelectedColumn = !'BobinaHoraSalida'
					&TFBobinaHoraSalida.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaHoraSalida_To.FromString(DDO_Grid.FilteredTextTo_get)
					If Not &TFBobinaHoraSalida_To.IsEmpty()
						&TFBobinaHoraSalida_To.Set(&TFBobinaHoraSalida_To.Year(), &TFBobinaHoraSalida_To.Month(), &TFBobinaHoraSalida_To.Day(), 23, 59, 59)
					EndIf
				Case DDO_Grid.SelectedColumn = !'BobinaDesviacionEstandar'
					&TFBobinaDesviacionEstandar.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaDesviacionEstandar_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaKg'
					&TFBobinaKg.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaKg_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaMermaKg'
					&TFBobinaMermaKg.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaMermaKg_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaNo'
					&TFBobinaNo.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaNo_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaReposoEnHoras'
					&TFBobinaReposoEnHoras.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaReposoEnHoras_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'ExtrusionOperadorNombre'
					&TFExtrusionOperadorNombre = DDO_Grid.FilteredText_get
					&TFExtrusionOperadorNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaObservaciones'
					&TFBobinaObservaciones = DDO_Grid.FilteredText_get
					&TFBobinaObservaciones_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaRechazadaObservaciones'
					&TFBobinaRechazadaObservaciones = DDO_Grid.FilteredText_get
					&TFBobinaRechazadaObservaciones_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaSiloMolidoNombre'
					&TFBobinaSiloMolidoNombre = DDO_Grid.FilteredText_get
					&TFBobinaSiloMolidoNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaSiloVirgenNombre'
					&TFBobinaSiloVirgenNombre = DDO_Grid.FilteredText_get
					&TFBobinaSiloVirgenNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaLoteVirgen'
					&TFBobinaLoteVirgen = DDO_Grid.FilteredText_get
					&TFBobinaLoteVirgen_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'ExtrusionLotePaqueteAditivos'
					&TFExtrusionLotePaqueteAditivos = DDO_Grid.FilteredText_get
					&TFExtrusionLotePaqueteAditivos_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaProductoId'
					&TFBobinaProductoId.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaProductoId_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaProductoNombre'
					&TFBobinaProductoNombre = DDO_Grid.FilteredText_get
					&TFBobinaProductoNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaProductoTipoMaterial'
					&TFBobinaProductoTipoMaterial_SelsJson = DDO_Grid.SelectedValue_get
					&TFBobinaProductoTipoMaterial_Sels.FromJson(&TFBobinaProductoTipoMaterial_SelsJson)
				Case DDO_Grid.SelectedColumn = !'BobinaPrensadoPrensaNombre'
					&TFBobinaPrensadoPrensaNombre = DDO_Grid.FilteredText_get
					&TFBobinaPrensadoPrensaNombre_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaInterrupcionesMotivo'
					&TFBobinaInterrupcionesMotivo = DDO_Grid.FilteredText_get
					&TFBobinaInterrupcionesMotivo_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaInterrupcionesTiempo'
					&TFBobinaInterrupcionesTiempo.FromString(DDO_Grid.FilteredText_get)
					&TFBobinaInterrupcionesTiempo_To.FromString(DDO_Grid.FilteredTextTo_get)
				Case DDO_Grid.SelectedColumn = !'BobinaDownTimeCode'
					&TFBobinaDownTimeCode = DDO_Grid.FilteredText_get
					&TFBobinaDownTimeCode_Sel = DDO_Grid.SelectedValue_get
				Case DDO_Grid.SelectedColumn = !'BobinaDownTimeDescription'
					&TFBobinaDownTimeDescription = DDO_Grid.FilteredText_get
					&TFBobinaDownTimeDescription_Sel = DDO_Grid.SelectedValue_get
			EndCase
			Grid.FirstPage()
	EndCase
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### Grid.Load (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	&Opciones.Clear()
	&Opciones.AddItem(0, 'Opciones')
	If (&IsAuthorized_Display)
		&Opciones.AddItem(1, format(!'%1;%2', "GXM_display", !"fa fa-search"))
	Endif
	If (&IsAuthorized_Update)
		&Opciones.AddItem(2, format(!'%1;%2', "GXM_update", !"fa fa-pen"))
	Endif
	If (&IsAuthorized_Delete)
		&Opciones.AddItem(3, format(!'%1;%2', "GX_BtnDelete", !"fa fa-times"))
	Endif
	If (BobinaEstado = EstadoBobina.EnProceso)
		&Opciones.AddItem(4, 'En Pausa')
	Endif
	&Opciones.AddItem(5, 'Etiqueta')
	
	If (&IsAuthorized_BobinaNoSerie)
		BobinaNoSerie.Link = DB.BobinaView.Link(BobinaId, '')
	Endif
	If (&IsAuthorized_ExtrusionExtrusoraNombre)
		ExtrusionExtrusoraNombre.Link = ExtrusionView.Link(ExtrusionId, '')
	Endif
	If (&IsAuthorized_BobinaInterrupcionesMotivo)
		BobinaInterrupcionesMotivo.Link = DB.InterrupcionView.Link(BobinaInterrupcionesId, '')
	Endif
	Do Case
		Case BobinaEstado = EstadoBobina.EnMedicion
			&GridBadge = "En Medición"
			&GridBadge.TooltipText = "En Medición"
			&GridBadge.Class = !' AttributeTagInfo BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Reposo
			&GridBadge = "Reposo"
			&GridBadge.TooltipText = "Reposo"
			&GridBadge.Class = !' AttributeTagInfoLight BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Molino
			&GridBadge = "Molino"
			&GridBadge.TooltipText = "Molino"
			&GridBadge.Class = !' AttributeTagDanger BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.EnProceso
			&GridBadge = "En Proceso"
			&GridBadge.TooltipText = "En Proceso"
			&GridBadge.Class = !' AttributeTagWarning BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Disponible
			&GridBadge = "Disponible"
			&GridBadge.TooltipText = "Disponible"
			&GridBadge.Class = !' AttributeTagSuccess BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.EnPrensado
			&GridBadge = "En Prensado"
			&GridBadge.TooltipText = "En Prensado"
			&GridBadge.Class = !' AttributeTagWarning BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Pausada
			&GridBadge = "Pausada"
			&GridBadge.TooltipText = "Pausada"
			&GridBadge.Class = !' AttributeTagWarning BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Desmontada
			&GridBadge = "Desmontada"
			&GridBadge.TooltipText = "Desmontada"
			&GridBadge.Class = !' AttributeTagInfo BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Transferida
			&GridBadge = "Transferida"
			&GridBadge.TooltipText = "Transferida"
			&GridBadge.Class = !' AttributeTagInfo BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Rechazada
			&GridBadge = "Rechazada"
			&GridBadge.TooltipText = "Rechazada"
			&GridBadge.Class = !' AttributeTagDanger BootstrapTooltipRight'
		Case BobinaEstado = EstadoBobina.Consumida
			&GridBadge = "Consumida"
			&GridBadge.TooltipText = "Consumida"
			&GridBadge.Class = !' AttributeTagDanger BootstrapTooltipRight'
		Otherwise
			&GridBadge = ''
			&GridBadge.TooltipText = ''
			&GridBadge.Class = !''
	EndCase
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### DDO_GridColumnsSelector.OnColumnsChanged (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	&ColumnsSelectorXML = DDO_GridColumnsSelector.ColumnsSelectorValues
	&ColumnsSelector.FromJson(&ColumnsSelectorXML)
	SaveColumnsSelectorState(!'Produccion.vwAnaliticaBobinaColumnsSelector', iif(&ColumnsSelectorXML.IsEmpty(), '', &ColumnsSelector.ToXml()))
	Refresh
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### ddo_ManageFilters.OnOptionClicked (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	Do Case
		Case ddo_ManageFilters.ActiveEventKey = !'<#Clean#>'
			Do 'CleanFilters'
			Grid.FirstPage()
			Refresh
		Case ddo_ManageFilters.ActiveEventKey = !'<#Save#>'
			Do 'SaveGridState'
			SaveFilterAs.Popup(!'Produccion.vwAnaliticaBobinaFilters', &PgmName + !"GridState")
			&ManageFiltersExecutionStep = 2
			Refresh
		Case ddo_ManageFilters.ActiveEventKey = !'<#Manage#>'
			ManageFilters.Popup(!'Produccion.vwAnaliticaBobinaFilters')
			&ManageFiltersExecutionStep = 2
			Refresh
		Otherwise
			&ManageFiltersXml = GetFilterByName(!'Produccion.vwAnaliticaBobinaFilters', ddo_ManageFilters.ActiveEventKey)
			If &ManageFiltersXml.IsEmpty()
				msg('WWP_FilterNotExist')
			Else
				Do 'CleanFilters'
				SaveGridState.Call(&PgmName + !"GridState",  &ManageFiltersXml)
				&GridState.FromXml(&ManageFiltersXml)
				&OrderedBy = &GridState.OrderedBy
				&OrderedDsc = &GridState.OrderedDsc
				Do 'SetDDOSortedStatus' 
				Do 'LoadRegFiltersState'
				Grid.FirstPage()
				Refresh
			EndIf
	EndCase
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### &Opciones.Click (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	Do Case
		Case &Opciones = 1
			Do 'Do Display'
		Case &Opciones = 2
			Do 'Do Update'
		Case &Opciones = 3
			Do 'Do Delete'
		Case &Opciones = 4
			Do 'Do EnPausa'
		Case &Opciones = 5
			Do 'Do cmdEtiqueta'
	EndCase
	&Opciones = 0
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### ddo_AGExport.OnOptionClicked (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	Do Case
		Case ddo_AGExport.ActiveEventKey = !'Export'
			Do 'DoExport'
		Case ddo_AGExport.ActiveEventKey = !'ExportReport'
			Do 'DoExportReport'
	EndCase
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### DDC_Subscriptions.OnLoadComponent (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	WWPAux_WC.Object = WWP_SubscriptionsPanel.Create(!'Bobina', WWPBaseObjects.Notifications.WWP_NotificationAppliesTo.AnyRecord, '', '')
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### 'DodeletedRecords' (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	If (&IsAuthorized_deletedRecords)
		WWPBaseObjects.AuditDeleted.Call(&AuditTableName)
	Else
		msg("WWP_ActionNoLongerAvailable")
		Refresh
	Endif
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### 'DoUserAction1' (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	DB.LlenadoBobinaInterrupcion.Call()
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### 'DocmdImprimir' (Event)

```genexus
/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */
	If (&IsAuthorized_cmdImprimir)
		admin.ImprimirBobinas.Call(&BobinaId_WSKey, &BobinaNoSerie_WSKey)
	Else
		msg("WWP_ActionNoLongerAvailable")
		Refresh
	Endif
	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

### Rules (Rules)

```genexus

	/* Generated by DVelop Work With Plus Pattern [Start] - Do not change */



	/* Generated by DVelop Work With Plus Pattern [End] - Do not change */
```

