...
import { ReportesService, ExtrusionResultadoItem } from '../../../../core/services/reportes';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-resumen-extrusion',
  standalone: true,
    ...
    <div class="module-page animate-move-up">
      ...
      <header class="module-header-container" style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;">
        <div class="title-area">
          <nav class="breadcrumb">Reportes HC › Resúmenes › Extrusión</nav>
          <h1 class="page-title">Resumen de Extrusión</h1>
        </div>
        ...
        <div class="actions-toolbar" style="display: flex; width: 100%; align-items: center; justify-content: space-between;">
          <div class="toolbar-left" style="display: flex; gap: 1.5rem; align-items: center;">
            <div class="search-box">
              <span class="search-label">Buscar por Extrusora/Producto/Operario</span>
            ...
            @if (loading()) {
              ...
              @for (item of filteredItems(); track item.id) {
                <tr>
                  ...
                  <td style="font-weight: 600; color: #1e293b;">{{ item.extrusora }}</td>
                  <td class="col-nombre">{{ item.producto }}</td>
                  <td>{{ item.turno }}</td>
                  <td>{{ item.operario }}</td>
                  ...
                  <td style="font-weight: 700; color: #15803d;">{{ item.totalKg | number:'1.2-2' }} kg</td>
                  <td>{{ item.siloNombre }} ({{ item.loteSilo }})</td>
                  <td>
                    <span class="status-badge" [ngClass]="{
                      'status-comba-si': item.comba,
                      'status-comba-no': !item.comba
                    }">
                      {{ item.comba ? 'Sí' : 'No' }}
                    </span>
                  </td>
...
export class ResumenExtrusionComponent implements OnInit {
  private reportesService = inject(ReportesService);

  searchTerm = '';
  loading = signal(false);
  items = signal<ExtrusionResultadoItem[]>([]);
  ...
  loadReport() {
    ...
    this.reportesService.getExtrusionResultadoReport().subscribe({
      next: (data) => {
        this.items.set(data);
        this.loading.set(false);
      },
      error: (err) => {
        console.error('Error cargando Resumen Extrusión:', err);
        this.loading.set(false);
      }
    });
  }
  ...
  exportExcel() {
    ...
    const dataToExport = this.filteredItems().map(item => ({
      ...
      'Bobinas Molino': item.bobinasMolino,
      ...
      'Comba': item.comba ? 'Sí' : 'No',
      'Observaciones': item.observaciones
    }));

    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(dataToExport);
    XLSX.utils.book_append_sheet(wb, ws, 'Extrusión');
    XLSX.writeFile(wb, 'Resumen_Extrusion.xlsx');
  }
  ...
  exportPDF() {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Resumen de Extrusión</title>
          <style>
        ...
        <body>
          <div class="header-area">
            <h1>Resumen Resultados de Extrusión</h1>
