...
import { ReportesService, PrensadoResultadoItem } from '../../../../core/services/reportes';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-resumen-prensado',
  standalone: true,
    ...
    <div class="module-page animate-move-up">
      ...
      <header class="module-header-container" style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;">
        <div class="title-area">
          <nav class="breadcrumb">Reportes HC › Resúmenes › Prensado</nav>
          <h1 class="page-title">Resumen de Prensado</h1>
        </div>
        ...
        <div class="actions-toolbar" style="display: flex; width: 100%; align-items: center; justify-content: space-between;">
          <div class="toolbar-left" style="display: flex; gap: 1.5rem; align-items: center;">
            <div class="search-box">
              <span class="search-label">Buscar por Prensa/Operario/Turno</span>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>Merma (KG)</th>
              <th>No. Palets</th>
              <th>Carretes Sobrantes</th>
              <th>RPM Línea</th>
              <th>GPM (Prensa/Total)</th>
              <th>Herramientas</th>
              <th>Levas (Medida/Entr/Sal)</th>
              <th>Rodillos (Medida/Entr/Sal)</th>
              <th>Observaciones</th>
            </tr>
          </thead>
          <tbody>
            @if (loading()) {
              <tr>
                <td colspan="15" class="empty-state">Cargando resultados de prensado...</td>
              </tr>
            } @else if (filteredItems().length === 0) {
              <tr>
                <td colspan="15" class="empty-state">No se encontraron registros de prensado</td>
              </tr>
            } @else {
              @for (item of filteredItems(); track item.id) {
                <tr>
                  <td>{{ item.fecha | date:'dd/MM/yyyy' }}</td>
                  <td style="font-weight: 600; color: #1e293b;">{{ item.prensa }}</td>
                  <td>{{ item.turno }}</td>
                  <td>{{ item.operario }}</td>
                  <td class="col-nombre">{{ item.piezasBuenas | number }} pzas</td>
                  <td style="color: #64748b;">{{ item.piezasMolino | number }} pzas</td>
                  <td style="color: #ef4444; font-weight: 600;">{{ item.mermaKg | number:'1.2-2' }} kg</td>
                  <td>{{ item.noPalets }}</td>
                  <td>{{ item.carretesSobrantes }}</td>
                  <td>{{ item.rpmLinea }}</td>
                  <td>{{ item.gpmPrensa }} / {{ item.gpmTotal }}</td>
                  <td>{{ item.herramientas }}</td>
                  <td style="font-size: 0.8rem; color: #475569;">
                    {{ item.levasUnidadMedida }}: IN {{ item.levasKgEntrada }}kg ({{ item.levasGradosEntrada }}°) / OUT {{ item.levasKgSalida }}kg ({{ item.levasGradosSalida }}°)
                  </td>
                  <td style="font-size: 0.8rem; color: #475569;">
                    {{ item.rodillosUnidadMedida }}: IN {{ item.rodillosKgEntrada }}kg ({{ item.rodillosGradosEntrada }}°) / OUT {{ item.rodillosKgSalida }}kg ({{ item.rodillosGradosSalida }}°)
                  </td>
...
export class ResumenPrensadoComponent implements OnInit {
  private reportesService = inject(ReportesService);

  searchTerm = '';
  loading = signal(false);
  items = signal<PrensadoResultadoItem[]>([]);
  ...
  filteredItems = computed(() => {
    ...
    return this.items().filter(item => 
      item.prensa.toLowerCase().includes(term) ||
      item.operario.toLowerCase().includes(term) ||
      item.turno.toLowerCase().includes(term) ||
      item.herramientas.toLowerCase().includes(term) ||
  ...
  loadReport() {
    ...
    this.reportesService.getPrensadoResultadoReport().subscribe({
      next: (data) => {
        this.items.set(data);
        this.loading.set(false);
      },
      error: (err) => {
        console.error('Error cargando Resumen Prensado:', err);
        this.loading.set(false);
      }
    });
  }
  ...
  exportExcel() {
    ...
    const dataToExport = this.filteredItems().map(item => ({
      ...
      'Merma (KG)': item.mermaKg,
      'No. Palets': item.noPalets,
      'Carretes Sobrantes': item.carretesSobrantes,
      'RPM Línea': item.rpmLinea,
      'GPM Prensa': item.gpmPrensa,
      'GPM Total': item.gpmTotal,
      'Herramientas': item.herramientas,
      'Levas UM': item.levasUnidadMedida,
      'Levas KG Entrada': item.levasKgEntrada,
      'Levas KG Salida': item.levasKgSalida,
      'Levas Grados Entrada': item.levasGradosEntrada,
      'Levas Grados Salida': item.levasGradosSalida,
      'Rodillos UM': item.rodillosUnidadMedida,
      'Rodillos KG Entrada': item.rodillosKgEntrada,
      'Rodillos KG Salida': item.rodillosKgSalida,
      'Rodillos Grados Entrada': item.rodillosGradosEntrada,
      'Rodillos Grados Salida': item.rodillosGradosSalida,
      'Observaciones': item.observaciones
    }));

    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(dataToExport);
    XLSX.utils.book_append_sheet(wb, ws, 'Prensado');
    XLSX.writeFile(wb, 'Resumen_Prensado.xlsx');
  }
  ...
  exportPDF() {
    ...
    this.filteredItems().forEach(item => {
      rows += `
        <tr>
          ...
          <td>${item.mermaKg.toFixed(1)}</td>
          <td>${item.noPalets}</td>
          <td>${item.carretesSobrantes}</td>
          <td>${item.rpmLinea}/${item.gpmPrensa}</td>
          <td>${item.herramientas}</td>
        </tr>
      `;
    });
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Resumen de Prensado</title>
          <style>
        ...
        <body>
          <div class="header-area">
            <h1>Resumen Resultados de Prensado</h1>
