...
import { ReportesService, CarretePalletItem } from '../../../../core/services/reportes';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-carrete-pallet',
  standalone: true,
    ...
    <div class="module-page animate-move-up">
      ...
      <header class="module-header-container" style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;">
        <div class="title-area">
          <nav class="breadcrumb">Reportes HC › Descargables › Carrete_Pallet</nav>
          <h1 class="page-title">Relación Carrete - Pallet</h1>
        </div>
        ...
        <div class="actions-toolbar" style="display: flex; width: 100%; align-items: center; justify-content: space-between;">
          <div class="toolbar-left" style="display: flex; gap: 1.5rem; align-items: center;">
            <div class="search-box">
              <span class="search-label">Buscar por Pallet/Carrete/Producto</span>
      ...
      <div class="content-card">
        <table class="data-table">
          <thead>
            <tr>
              <th>Hora Fin Ensamble</th>
              <th>No. Serie Pallet</th>
              <th>Tipo Pallet</th>
              <th>Producto</th>
              <th>No. Serie Carrete</th>
              <th>Estatus Pallet</th>
            </tr>
          </thead>
          <tbody>
            @if (loading()) {
              <tr>
                <td colspan="6" class="empty-state">Cargando registros de carretes y pallets...</td>
              </tr>
            } @else if (filteredItems().length === 0) {
              <tr>
                <td colspan="6" class="empty-state">No se encontraron registros de relación carrete-pallet</td>
              </tr>
            } @else {
              @for (item of filteredItems(); track item.id) {
                <tr>
                  <td>{{ item.horaFinEnsamble | date:'dd/MM/yyyy HH:mm:ss' }}</td>
                  <td style="font-weight: 600; color: #1e293b;">{{ item.paletNoSerie }}</td>
                  <td>{{ item.paletTipo }}</td>
                  <td class="col-nombre">{{ item.paletProductoNombre }}</td>
                  <td style="font-weight: 600; color: #475569;">{{ item.carreteNoSerie }}</td>
                  <td>
                    <span class="status-badge" [ngClass]="{
                      'status-activo': item.paletEstatus === 'Activo' || item.paletEstatus === 'COMPLETO',
                      'status-pendiente': item.paletEstatus === 'Pendiente' || item.paletEstatus === 'PROCESO'
                    }">
                      {{ item.paletEstatus }}
                    </span>
                  </td>
...
export class CarretePalletComponent implements OnInit {
  private reportesService = inject(ReportesService);

  searchTerm = '';
  loading = signal(false);
  items = signal<CarretePalletItem[]>([]);
  ...
  filteredItems = computed(() => {
    ...
    return this.items().filter(item => 
      item.paletNoSerie.toLowerCase().includes(term) ||
      item.paletTipo.toLowerCase().includes(term) ||
      item.paletProductoNombre.toLowerCase().includes(term) ||
      item.carreteNoSerie.toLowerCase().includes(term) ||
      (item.paletEstatus && item.paletEstatus.toLowerCase().includes(term))
    );
  });
  ...
  loadReport() {
    ...
    this.reportesService.getCarretePalletReport().subscribe({
      next: (data) => {
        this.items.set(data);
        this.loading.set(false);
      },
      error: (err) => {
        console.error('Error cargando Carrete-Pallet:', err);
        this.loading.set(false);
      }
    });
  }
  ...
  exportExcel() {
    ...
    const dataToExport = this.filteredItems().map(item => ({
      'Hora Fin Ensamble': item.horaFinEnsamble ? new Date(item.horaFinEnsamble).toLocaleString() : '',
      'No. Serie Pallet': item.paletNoSerie,
      'Tipo Pallet': item.paletTipo,
      'Producto': item.paletProductoNombre,
      'No. Serie Carrete': item.carreteNoSerie,
      'Estatus Pallet': item.paletEstatus
    }));

    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(dataToExport);
    XLSX.utils.book_append_sheet(wb, ws, 'Carrete-Pallet');
    XLSX.writeFile(wb, 'Reporte_Carrete_Pallet.xlsx');
  }
  ...
  exportPDF() {
    ...
    this.filteredItems().forEach(item => {
      rows += `
        <tr>
          <td>${item.horaFinEnsamble ? new Date(item.horaFinEnsamble).toLocaleString() : ''}</td>
          <td>${item.paletNoSerie}</td>
          <td>${item.paletTipo}</td>
          <td>${item.paletProductoNombre}</td>
          <td>${item.carreteNoSerie}</td>
          <td>${item.paletEstatus}</td>
        </tr>
      `;
    });
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Reporte Relación Carrete - Pallet</title>
          <style>
        ...
        <body>
          <div class="header-area">
            <h1>Reporte Relación Carrete - Pallet</h1>
          ...
          <table>
            ...
            <tbody>
              \${rows ? rows : '<tr><td colspan="6" style="text-align:center; font-style:italic;">No se encontraron registros</td></tr>'}
            </tbody>
          </table>
