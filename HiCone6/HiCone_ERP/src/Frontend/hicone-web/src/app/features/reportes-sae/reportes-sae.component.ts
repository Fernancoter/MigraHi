          ...
          <table class="data-table">
            ...
            <tbody>
              <tr *ngFor="let p of pedidos">
                ...
                <td>{{ p.claveCliente }}</td>
                <td>{{ p.fecha | date:'shortDate' }}</td>
                <td class="bold">{{ p.importe | currency }}</td>
                <td>
                  <span class="status-dot" [class]="p.estatus.toLowerCase()"></span>
                  {{ p.estatus === 'O' ? 'Original' : 'Parcial' }}
                </td>
              </tr>
            </tbody>
          </table>
...
export class ReportesSaeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.saeService.getReporteFTB(2024, 4).subscribe(data => this.presupuestos = data);
  }
