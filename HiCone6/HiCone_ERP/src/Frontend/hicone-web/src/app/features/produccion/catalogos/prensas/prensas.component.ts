    ...
    <div class="module-page animate-move-up">
      ...
      <div class="content-card" style="margin-top: 1rem; position: relative; min-height: 300px;">
        <table class="data-table">
          ...
          <tbody>
            @if (loading()) { 
              <tr><td colspan="6" class="empty-state">Cargando prensas...</td></tr> 
            }
            @else if (paginatedItems().length === 0) { 
              <tr><td colspan="6" class="empty-state">No se encontraron prensas registradas</td></tr> 
            }
            @else {
            ...
            <div class="modal-body">
              <div class="form-row">
                ...
                <input 
                  ...
                  placeholder="Nombre de la prensa..." 
                />
              </div>
              ...
              <div class="form-row">
                ...
                <input 
                  ...
                  [(ngModel)]="form.marca" 
                  [disabled]="modalReadOnly()" 
                  placeholder="Marca de la prensa..." 
                />
              </div>
              ...
              <div class="form-row">
                ...
                <input 
                  ...
                  [(ngModel)]="form.modelo" 
                  [disabled]="modalReadOnly()" 
                  placeholder="Modelo de la prensa..." 
                />
              </div>
            </div>
...
export class PrensasCatalogoComponent implements OnInit {
  ...
  save() {
    ...
    if (!this.form.id) {
      this.svc.createPrensa(this.form).subscribe(() => {
        this.closeModal();
        this.load();
      });
    } else {
      this.svc.updatePrensa(this.form.id, this.form).subscribe(() => {
        this.closeModal();
        this.load();
      });
    }
  ...
  del(item: Prensa) {
    if (confirm(`¿Está seguro de que desea eliminar la prensa "${item.nombre}"?`)) {
  ...
  exportCSV() {
    ...
    const headers = [];
    if (this.isColVisible('nombre')) headers.push('Prensa');
    if (this.isColVisible('marca')) headers.push('Marca');
    if (this.isColVisible('modelo')) headers.push('Modelo');
    csvContent += headers.join(';') + '\n';
    
    this.filteredItems().forEach(op => {
      const row = [];
      if (this.isColVisible('nombre')) row.push(op.nombre);
      if (this.isColVisible('marca')) row.push(op.marca || '');
      if (this.isColVisible('modelo')) row.push(op.modelo || '');
      csvContent += row.join(';') + '\n';
    });
    ...
    link.setAttribute('download', `prensas_reporte_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  exportPDF() {
    ...
    let headers = '';
    if (this.isColVisible('nombre')) headers += `<th>Prensa</th>`;
    if (this.isColVisible('marca')) headers += `<th>Marca</th>`;
    if (this.isColVisible('modelo')) headers += `<th>Modelo</th>`;

    let tableRows = '';
    this.filteredItems().forEach(op => {
      tableRows += `<tr>`;
      if (this.isColVisible('nombre')) tableRows += `<td>${op.nombre}</td>`;
      if (this.isColVisible('marca')) tableRows += `<td>${op.marca || ''}</td>`;
      if (this.isColVisible('modelo')) tableRows += `<td>${op.modelo || ''}</td>`;
      tableRows += `</tr>`;
    });
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Reporte de Prensas</title>
          <style>
