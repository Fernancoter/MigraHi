    ...
    <div class="module-page animate-move-up">
      ...
      @if (showSearch()) {
        <div style="margin-bottom:1rem">
          <input class="search-input" type="text" placeholder="Buscar prensa..." [(ngModel)]="searchTerm" (input)="load(searchTerm)" />
        </div>
      }
      <div class="content-card">
        <table class="data-table">
          <thead><tr><th colspan="3"></th><th>Prensa ↑</th><th>Marca ↑</th><th>Modelo ↑</th></tr></thead>
          <tbody>
            ...
            @else {
              @for (item of items(); track item.id) {
                <tr>
                  ...
                  <td>{{ item.nombre }}</td><td>{{ item.marca }}</td><td>{{ item.modelo }}</td>
                </tr>
              }
            }
          </tbody>
        ...
        <div class="modal-overlay" (click)="closeModal()">
          <div class="modal-card" (click)="$event.stopPropagation()">
            <div class="modal-header">
              <h3>{{ modalMode()==='create'?'Nueva Prensa':modalMode()==='edit'?'Modificar Prensa':'Prensa' }}</h3>
              <button class="modal-close" (click)="closeModal()">✕</button>
            </div>
            <div class="modal-body">
              <label class="field-label">Nombre *</label>
              <input class="field-input" type="text" [(ngModel)]="form.nombre" [disabled]="modalMode()==='view'" placeholder="Ej: Prensa 1" />
              <label class="field-label">Marca</label>
              <input class="field-input" type="text" [(ngModel)]="form.marca" [disabled]="modalMode()==='view'" />
              <label class="field-label">Modelo</label>
              <input class="field-input" type="text" [(ngModel)]="form.modelo" [disabled]="modalMode()==='view'" />
            </div>
