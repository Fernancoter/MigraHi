...
import { ProduccionConfigService, Extrusora, Turno } from '../../../../core/services/produccion-config.service';
    ...
    <div class="module-page animate-move-up">
      ...
      <header class="module-header-container" style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;">
        <div class="title-area">
          ...
          <h1 class="page-title">Extrusora Observacion</h1>
        </div>
        ...
        <div class="actions-toolbar" style="display: flex; width: 100%; align-items: center;">
          ...
          <div class="toolbar-left" style="display: flex; gap: 0.75rem; align-items: center;">
            <!-- Botón Agregar (Text only: Agregar) -->
            ...
            <!-- Botón Filtro (Funnel with popover options) -->
            <div class="dropdown-wrapper">
            ...
            @if (loading()) {
              ...
              @for (item of paginatedItems(); track item.id) {
                <tr>
                  ...
                  @if (isColVisible('extrusora')) { <td class="col-nombre">{{ item.extrusora }}</td> }
          ...
          <div class="modal-card detailed-modal" (click)="$event.stopPropagation()">
            <div class="modal-header">
              <div class="modal-title-area">
                <nav class="breadcrumb">Reportes HC › Observaciones › Extrusoras</nav>
                <h1 class="modal-title">Extrusora Observacion</h1>
              </div>
                ...
                <div class="panel-content">
                  ...
                  <div class="modal-grid-2">
                    <div class="form-row">
                      ...
                      <select [(ngModel)]="form.extrusoraId" class="field-input-line select-arrow">
                        <option value="">-- Seleccionar Extrusora --</option>
                        @for (ext of extrusorasList(); track ext.id) {
        ...
        <div class="modal-overlay" style="z-index: 1100;" (click)="closeArchiveModal()">
          <div class="modal-card archive-modal" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body archive-body">
              <p>¿Está seguro de archivar la observación de extrusora?</p>
            </div>
...
export class ExtrusorasObservacionComponent implements OnInit {
  ...
  // Dropdown catalogs
  extrusorasList = signal<Extrusora[]>([]);
  ...
  itemToArchive = signal<ExtrusoraObservacion | null>(null);
  ...
  load() {
    ...
    this.svc.getExtrusorasObservaciones().subscribe({
      next: data => {
        this.items.set(data);
  ...
  loadCatalogs() {
    this.svc.getExtrusoras().subscribe(data => this.extrusorasList.set(data));
  ...
  filteredItems = computed(() => {
    ...
    if (search) {
      list = list.filter(item =>
        ...
        item.extrusora.toLowerCase().includes(search) ||
  ...
  openEditModal(item: ExtrusoraObservacion) {
    const extrusoraObj = this.extrusorasList().find(e => e.nombre.toLowerCase() === item.extrusora.toLowerCase());
    ...
    this.form = {
      ...
      extrusoraId: extrusoraObj ? extrusoraObj.id : '',
  ...
  confirmArchive(item: ExtrusoraObservacion) {
  ...
  save() {
    if (!this.form.extrusoraId) {
      alert('Debe seleccionar una Extrusora.');
      return;
    }
    ...
    const extrusoraNombre = this.extrusorasList().find(e => e.id === this.form.extrusoraId)?.nombre || '';
    const turnoNombre = this.turnosList().find(t => t.id === this.form.turnoId)?.nombre || '';

    if (!this.form.id) {
      // In-memory insert
      const newObj: ExtrusoraObservacion = {
        id: Date.now(),
        fecha: this.form.fecha,
        extrusora: extrusoraNombre,
        turno: turnoNombre,
        interrupcion: '0.00',
  ...
  exportExcel() {
    ...
    const dataToExport = this.filteredItems().map(item => {
      ...
      if (this.isColVisible('extrusora')) row['Extrusora'] = item.extrusora;
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Observaciones Extrusoras');

    XLSX.writeFile(wb, `observaciones_extrusoras_${new Date().toISOString().slice(0,10)}.xlsx`);
  }
  ...
  exportPDF() {
    ...
    this.filteredItems().forEach(item => {
      ...
      if (this.isColVisible('extrusora')) tableRows += `<td>${item.extrusora}</td>`;
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Reporte de Observaciones de Extrusoras</title>
          <style>
        ...
        <body>
          <h1>Reporte de Observaciones de Extrusoras</h1>
