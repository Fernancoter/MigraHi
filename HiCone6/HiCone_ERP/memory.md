...
## 2026-07-22: Corrección en Configuración de Operarios de Extrusora y Sembrador de BD
...
- **Sembrador de Base de Datos**: Se detectó y corrigió un error en el sembrador de base de datos (`ApplicationDbContextSeeder.cs`) que fallaba en entornos limpios o vacíos al intentar sembrar las extrusoras y prensas iniciales de demostración sin definir sus campos obligatorios `NumeroExtrusora` / `NumeroPrensa` (mapeados a la columna `codigo` en BD). Se agregaron estos campos por defecto para prevenir errores `NOT NULL` en base de datos.
